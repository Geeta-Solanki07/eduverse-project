...
export default function Navbar() {
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case "elementary": return "text-orange-500 hover:text-orange-600";
      case "junior": return "text-green-600 hover:text-green-500";
      case "senior": return "text-blue-600 hover:text-blue-500";
    ...
    <nav className="flex items-center justify-between px-5 md:px-10 py-4 bg-white/90 backdrop-blur-md shadow-md sticky top-0 z-50">
      ...
      <div className="hidden md:flex flex-1 justify-center items-center gap-6">
        ...
        <div className="relative group">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 top-full mt-3 hidden group-hover:grid grid-cols-3 gap-6 bg-white rounded-2xl shadow-2xl p-6 w-[900px] border border-gray-100 transition-all duration-300">
        ...
        {/* Other Links */}
        <Link href="/course/academics" className="font-semibold text-gray-700 hover:text-orange-500  md:rounded-lg md:px-3 md:py-1 transition">
          Study Materials
        </Link>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-white shadow-lg p-6 flex flex-col gap-4 md:hidden transition-all duration-300">
          {classes.map((c) => (
            <Link key={c._id} href={`/academics/${c.slug}`} className={`font-medium ${getCategoryColor(c.category)}`} onClick={() => setMenuOpen(false)}>
              {c.title}
            </Link>
          ))}

          <Link href="/academics/study-materials2" className="font-medium text-gray-800 hover:text-orange-500" onClick={() => setMenuOpen(false)}>
            Study Materials
          </Link>

          <Link href="/support" className="font-medium text-gray-800 hover:text-orange-500" onClick={() => setMenuOpen(false)}>
            Support
          </Link>

          <Link href="/auth/login" className="bg-orange-500 text-white py-2 rounded-lg text-center font-semibold" onClick={() => setMenuOpen(false)}>
            Login / Register
          </Link>
        </div>
      )}
